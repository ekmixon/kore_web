...
class InstallRequirement(object):
    ...
    def from_line(
        ...
        if marker_sep in name:
            ...
            markers = Marker(markers) if markers else None
        else:
            markers = None
        name = name.strip()
        req = None
        ...
        if link:
            ...
            if link.is_wheel:
                ...
                req = f"{wheel.name}=={wheel.version}"
            else:
        ...
        extras = Requirement(f"placeholder{extras.lower()}").extras if extras else ()
        if req is not None:
            try:
                req = Requirement(req)
            except InvalidRequirement:
    ...
    def egg_info_data(self, filename):
        if self.satisfied_by is not None:
            return (
                self.satisfied_by.get_metadata(filename)
                if self.satisfied_by.has_metadata(filename)
                else None
            )

        assert self.source_dir
        filename = self.egg_info_path(filename)
        return read_text_file(filename) if os.path.exists(filename) else None
    ...
    def egg_info_path(self, filename):
        if self._egg_info_path is None:
            ...
            if not filenames:
                raise InstallationError(f'No files/directories in {base} (from {filename})')
            assert filenames, f"No files/directories in {base} (from {filename})"
    ...
    def archive(self, build_dir):
        assert self.source_dir
        create_archive = True
        archive_name = f'{self.name}-{self.pkg_info()["version"]}.zip'
        ...
        if create_archive:
            ...
            for dirpath, dirnames, filenames in os.walk(dir):
                ...
                for dirname in dirnames:
                    ...
                    zipdir = zipfile.ZipInfo(f'{self.name}/{name}/')
                ...
                for filename in filenames:
                    ...
                    zip.write(filename, f'{self.name}/{name}')
            zip.close()
...
def _strip_postfix(req):
    ...
    """
    if match := re.search(r'^(.*?)(?:-dev|-\d.*)$', req):
