...
class ListCommand(Command):
    ...
    def output_legacy(self, dist, options):
        if options.verbose >= 1:
            return f'{dist.project_name} ({dist.version}, {dist.location}, {get_installer(dist)})'

        elif dist_is_editable(dist):
            return f'{dist.project_name} ({dist.version}, {dist.location})'
        else:
            return f'{dist.project_name} ({dist.version})'

    def output_legacy_latest(self, dist, options):
        return f'{self.output_legacy(dist, options)} - Latest: {dist.latest_version} [{dist.latest_filetype}]'
